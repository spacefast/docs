...
store that [Zero](/zero#storage) uses. Objects belong to the space and
survive rollbacks. The binding has three methods:
...
`env.DB.exec()` runs DDL (data definition language) statements, and
`env.DB.batch()` groups statements. `sf db console` opens a full SQL
session.
...
Functions do not run inside `sf dev` yet. Use your framework's dev server,
