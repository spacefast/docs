...
Add `_redirects` at the publish root to redirect, rewrite, proxy, or return custom 404 responses. Spacefast compiles the file at deploy finalize and attaches the rules to that immutable version. Serving is precomputed — no rule is parsed or looked up per request, and the file itself is never served.
...
Rewrite and redirect rules are a plan limit: 10 on Free, 1,000 on Personal, unlimited on Work. Going over warns before it blocks, and limits only move up. The compiler still parses everything you publish, so diagnostics and upgrade prompts point at exact rules.
...
External proxy rules — `200` with an absolute URL destination — are not available on Free. Available on Personal and Work. The rule still compiles on Free; it just does not execute until you upgrade. See [Proxy routes](/proxy-routes).
...
Custom response headers use a separate [Headers](/headers) page. Proxy examples are covered in [Proxy routes](/proxy-routes).
