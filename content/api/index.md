...
The API ships a machine-discovery surface — [`llms.txt`](https://spacefast.com/llms.txt), an [agent card](https://spacefast.com/.well-known/agent-card.json), a publish skill, and a hosted MCP server with typed tools. If an agent is doing the calling, start at [Agents](/agents).
