...
:::note[Early access]
Tag Manager is in private beta; ask Spacefast to enable the `tags` feature
for your team. The CLI hides `sf tags` from help until you set
`SPACEFAST_CLI_EARLY_ACCESS=tags`; the commands run either way, and the API
...
category. Use `sf tags templates` for the current list of providers.
...
In automation, pass `--expected-current-version` so concurrent changes fail
...
Revoke the session when you finish the review:
