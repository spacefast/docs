...
- **Audience**: Public, Team, Person, Link, Password, Machine, or an
  external identity.
...
Visitors can request access from the private-space access page. List
pending requests, then approve or deny:
...
- **Open** links carry their secret in a URL fragment on the handoff
  ...
  handoff, and the serving host sets a `__Host-` cookie before redirecting
  to the clean live URL.
- **Link** URLs are served on the space's own host as
  `https://<space-host>/<landing>?__=<token>`, with no redirect and no
  interstitial page.
