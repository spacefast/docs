...
Spacefast publishes plain Vite and TanStack Start static output without framework-specific packages. Use `sf.jsonc`, `_redirects`, and `_headers` for deploy behavior. The optional Spacefast Vite plugin is only for local routing parity and build validation.
