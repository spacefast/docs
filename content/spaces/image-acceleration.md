...
- Site Accelerator serves common raster image formats such as `.gif`, `.png`,
  `.jpg`, and `.webp`.
- Site Accelerator converts to more efficient lossy output (WebP) when the
  request and the browser support it.

## URL helper package

Use `@spacefast/image` when a framework integration needs more than width and
quality. It always emits `i0.wp.com` URLs instead of distributing requests
across hosts, so repeated requests for the same transformed image share one
cache key.
...
- Site Accelerator keys cached images by image URL. When an image changes,
  change the filename or the path so the edge cache fetches the new asset. Do
  not rely on query-string cache busters on the original image URL.
- Site Accelerator is for images and static assets. It does not serve video,
  audio, or other media formats as image transforms.
- Site Accelerator may not apply heavy filters such as `brightness`,
  `contrast`, `colorize`, and `smooth` to large images.
