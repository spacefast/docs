...
`github-copilot` and `factory-droid` get skills plus MCP. `gemini-cli`,
`devin-desktop`, `cline`, and `continue` are skills-only and need
...
connects with the connector deeplink on [its setup page](/setup/claude-app),
and the desktop app has [its own page](/setup/claude-desktop). Any other
