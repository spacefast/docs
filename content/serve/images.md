...
- Site Accelerator serves common raster image formats such as `.gif`,
  `.png`, `.jpg`, and `.webp`.
...
Use `@spacefast/image` when a framework integration needs more than width
and quality. It always emits `i0.wp.com` URLs instead of distributing
requests across hosts, so repeated requests for the same transformed image
share one cache key.
