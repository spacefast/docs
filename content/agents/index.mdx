...
Cursor, VS Code, Poke). Each agent's page under
[spacefast.com/setup](https://spacefast.com/setup) ranks its best paths.
The rest of this page covers the CLI, the setup command, skills, and MCP.
...
From the project the agent should work in, run:
...
configures MCP. Use `sf setup agent -y` for a non-interactive setup, or
pick one client and hosted MCP explicitly:
...
with the connector deeplink in the table. Any other agent that can fetch a
URL can use [hosted MCP](/agents/mcp) or the
...
redeemed with `sf login --handoff`. Never paste secrets into prompts.
