...
Put server-only values in `.env.server` or manage them with [`sf env`](/spaces/variables).
...
routes SSR and route handlers to the worker. See [Next.js](/publishing/frameworks/nextjs)
